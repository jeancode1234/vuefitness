<template>
  <div>
    <div class="w-full p-4 bg-black fixed z-20 flex justify-between">
      <div class="flex justify-center">
        <h1 class="text-3xl font-extrabold text-center text-white uppercase italic">Dim<span
            class="text-red-600">Fitness</span></h1>
      </div>

      <nav :class="{ 'hidden': !isMenuOpen, 'flex': isMenuOpen }"
        class="flex-col md:flex md:flex-row md:items-center md:justify-center space-y-4 md:space-y-0 md:space-x-10 font-semibold text-lg text-white">
        ...
        <router-link to="/dashboard"
          class="px-6 py-3 rounded-lg bg-red-600 hover:bg-red-700 text-white shadow">Profil</router-link>
      </div>
      <div class="flex items-center justify-between space-x-4" v-if="!isConnected">
        <router-link to="/login"
          class="px-6 py-3 rounded-lg bg-red-600 hover:bg-red-700 text-white shadow">Connexion</router-link>
        <router-link to="/register"
          class="px-6 py-3 rounded-lg border border-red-600 hover:bg-red-700 text-white shadow">S'inscrire</router-link>
      </div>
      </nav>
